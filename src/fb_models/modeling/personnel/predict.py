...
from .features import PERSONNEL_CATEGORICAL_COLS, PERSONNEL_FEATURE_COLS
...
_COACH_TENDENCY_IS_OFFENSE = {
    ...
    "off_under_center_rate_hist": True,
    "off_pistol_rate_hist": True,
    "off_no_huddle_rate_hist": True,
    "off_11_personnel_rate_hist": True,
    "off_12_personnel_rate_hist": True,
    "off_21_personnel_rate_hist": True,
    "off_13_personnel_rate_hist": True,
    "off_22_personnel_rate_hist": True,
    "off_01_personnel_rate_hist": True,
...
def build_live_feature_row(
    *,
    ...
    predict_personnel_probs from raw simulation game-state values.

    play_type and offense_formation are the already-decided calls from the
    upstream models -- personnel is predicted downstream of both in the
    pipeline (play_type -> formation -> personnel -> result).

    Reuses the same derived-column thresholds/bucket bins as dataset.py
    and the same coach_snapshot shape as play_type/formation's
    build_live_feature_row (fb_models.modeling.plays.build_coach_snapshot),
    so this can't drift from what the model was actually trained on.
    """
    row: dict = {
        "down": down,
        "ydstogo": ydstogo,
    ...
    return df[PERSONNEL_FEATURE_COLS]


def predict_personnel_probs(
    clf: lgb.LGBMClassifier, game_state: pd.DataFrame
) -> dict[str, float]:
    probs = clf.predict_proba(game_state[PERSONNEL_FEATURE_COLS])[0]  # type: ignore
