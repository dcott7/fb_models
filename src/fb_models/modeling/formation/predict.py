...
from .features import FORMATION_CATEGORICAL_COLS, FORMATION_FEATURE_COLS
...
def build_live_feature_row(
    *,
    ...
    predict_formation_probs from raw simulation game-state values.

    play_type is the already-decided run/pass call from the play_type
    model -- formation is predicted downstream of it in the pipeline
    (play_type -> formation -> personnel -> result).

    Reuses the same derived-column thresholds/bucket bins as dataset.py
    and the same coach_snapshot shape as play_type's build_live_feature_row
    (fb_models.modeling.plays.build_coach_snapshot), so this can't drift
    ...
    return df[FORMATION_FEATURE_COLS]


def predict_formation_probs(
    clf: lgb.LGBMClassifier, game_state: pd.DataFrame
) -> dict[str, float]:
    probs = clf.predict_proba(game_state[FORMATION_FEATURE_COLS])[0]  # type: ignore
